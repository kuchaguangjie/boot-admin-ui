...
import * as taskJobApi from "@/api/sys/taskJob";
import type { FormItemProps } from "./types";
import { addDialog } from "@/components/ReDialog";
import editForm from "../form.vue";
import { message } from "@/utils/message";

export function useTaskJob() {
  const { tagStyle } = usePublicHooks();
  const formRef = ref();

  const permission = reactive({
    query: ["sys:quartz:job:page"],
    add: ["sys:quartz:job:save"],
    edit: ["sys:quartz:job:update"],
    ...
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            if (curData.id) {
              const { success } = await taskJobApi.updateTaskJob(curData);
              if (success) {
                chores();
              }
            } else {
              const { success } = await taskJobApi.saveTaskJob(curData);
              if (success) {
                chores();
              }
            }
  ...
  async function onSearch() {
    ...
    const { success, data } = await taskJobApi
      .pageTaskJob(params)
      .finally(() => {
  ...
  async function handleRun(row: FormItemProps) {
    const { success } = await taskJobApi.RunTaskJob(row.id);
    if (success) {
      message(`任务： ${row.jobName} 执行成功`, { type: "success" });
    }
  }
  async function handleResume(row: FormItemProps) {
    const { success } = await taskJobApi.ResumeTaskJob(row.id);
    if (success) {
  ...
  async function handlePause(row: FormItemProps) {
    const { success } = await taskJobApi.PauseTaskJob(row.id);
    if (success) {
  ...
  async function handleDelete(row: FormItemProps) {
    const { success } = await taskJobApi.DeleteTaskJob(row.id);
    if (success) {
