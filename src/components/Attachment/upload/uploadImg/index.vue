...
import { UploadImageProps, FileTypes } from "./type";
import { ref } from "vue";
...
const id = ref(buildPrefixUUID("upload-image"));
...
const beforeUpload: UploadProps["beforeUpload"] = (rawFile: File) => {
  ...
  if (!imgType.includes(rawFile.type as FileTypes)) {
    message("上传图片不符合所需的格式！", { type: "warning" });
    return;
  }
  if (!imgSize) {
    message(`上传图片大小不能超过${props.fileSize}MB！`, { type: "warning" });
    return;
  }
